...
`pvm/README.md` carries the achieved-acceleration matrix (first
solution, counting, knapsack B&B, the (K,K)-Ramsey cells including the
$R(4,4)=18$ proof) with complete run commands and the per-sample
parameter/env reference.

Builtins: `bp.pvm_fork(NT, Mode, C)` arms the session, and
`bp.pvm_report(S)` / `bp.pvm_collect(R)` / `bp.pvm_solution(S)`
finish it. Mode 1 = the $C=1$ OR split; `bp.pvm_fork_rw(NT, M)` = first-wins mode 2 (race M candidates up front, first report wins by CAS, the losers are killed); mode 2 = static value-chunk
...
| `bp.pvm_fork_rw(NT, M)` | **First-wins mode 2** (`rw` = race-wins): the static chunk split forks the M candidates up front and runs them concurrently; the FIRST `pvm_report` wins by CAS on `found` and serializes the solution; `pvm_collect` waits for the winner's completion marker, SIGKILLs the still-running losers and returns early. This is the parblock race-family substrate; plain mode-2 count-all is untouched. |
...
| `bp.pvm_claim(Max, V)` | Mode 2, dynamic chunks: atomically claims the next value of the session's `1..Max` work set from the shared cursor (fetch-add; the first Max calls return exactly 1..Max each once, later calls return 0 = exhausted). A pool of workers loops claim--solve--`pvm_report` and balances uneven slice costs without re-forking; the static `pvm_chunk` range is ignored by such a driver (the R(4,4) cell runs it in `pvm/ramsey_m2.pi` with `DYN=1`, 2.10x over the static partition). |
| `bp.pvm_report(T)` | Mode 1/3 (and first-wins mode 2, `pvm_fork_rw`): report the solution **by value** — T may be any ground term (ints, atoms, lists, compounds, arrays; floats/bigints ride as their `$float`/`$bigint` PSCs); the first reporter wins by CAS, a second reporter is a no-op. Mode 2: any ground term, appended to the session's result buffer (as many times as it likes). |
| `bp.pvm_collect(R)` | Ends the session: SIGKILL sweep of the pool (after waiting for the solution's completion marker), blocking reaps, copy-out and unmap of the shared block. Mode 1/3 (and first-wins mode 2, `pvm_fork_rw`): R = 1 iff a solution was reported, else 0. Mode 2: R = the list of all reported terms, in report order (the parent consumes/sums it explicitly). Refuses the session as a hard `run_time_error` on a crashed worker or a `found` without a completed marker. A forked child reaching this in a finished mode-1/3 session exits quietly; a mode-2 worker reaching it (its report branch failed upstream) instead flags the session and drops out -- the real root's collect then refuses with `run_time_error` rather than serving a silent undercount or letting the worker run a second collect against the shared block. |
...
| `pvm/ramsey_m2.pi` | env `K= N= T= M= O= DYN=` (defaults 3, 6, 0, 0, 0, 0) | the $R(4,4) \le 18$ whole-tree UNSAT proof via a mode-2 joint partition of the first M edge variables: static `K=4 N=18 T=256 M=8` (256 workers, one 8-bit pattern each, 112-141 s depending on node load) vs dynamic `K=4 N=18 T=384 M=14 DYN=1` (a 384-worker pool claims the 16,384 fourteen-bit leaves from the shared `pvm_claim` cursor, 53.3 s quiet / 61.5 s hot = 2.10-2.30x); `O=` windows a sub-range, T=0 = serial oracle. See `pvm/README.md` for the full measured matrix and run commands |
