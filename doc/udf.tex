...
The include file \texttt{lib/udf.ops.pi} provides a one-line shim for
every arithmetic and Boolean constraint operator
(\verb+#= #!= #< #=< #<= #> #>=+ and
\verb+#~ #/\\ #^ #\\/ #=> #<=>+).  Adding
...
Two examples are provided in \texttt{exs/cp/}:
\texttt{udf\_functions.pi} (a simple ``normal \texttt{\#=}'' example) and
\texttt{udf\_ops\_schedule.pi} (a three-day production/stock scheduling
problem that minimises total production, using a recursive user function
\texttt{day\_demand/1} under \texttt{\#=} and \texttt{\#>=}).

To run them, the \texttt{udf} module must be found on
