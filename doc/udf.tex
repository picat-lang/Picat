...
The \texttt{udf}\index{\texttt{udf}} module (\texttt{lib2/udf.pi}) adds
...
\texttt{lib2/udf.ops.pi} (cp), \texttt{udf.ops\_sat.pi},
...
\begin{verbatim}
    picat -path <picat>/lib2  udf_functions.pi
    PICATPATH=<picat>/lib2  <picat>/emu/picat  udf_ops_schedule.pi
\end{verbatim}

where \texttt{<picat>/lib2} is the repository's precompiled standard
\texttt{lib2} directory (the udf operator file the examples
\texttt{include}, \texttt{udf\_ops*.pi}, sits next to the examples and is
found directly).
