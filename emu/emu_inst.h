...
lab_throw_ball:
    ...
    op2 = (BPLONG)NextOperandY;
    {
        ...
        BPLONG_PTR b = B;
        bp_stacktrace_lst = nil_sym;
        while ((BPLONG)af != AR_AR(af)) {
            btm_ptr = (BPLONG_PTR)UNTAGGED_ADDR(AR_BTM(af));
            if (btm_ptr > af){
                op3 = FOLLOW(btm_ptr);
                DEREF(op3);
                if (ISSTRUCT(op3)){
                    *(H) = op3;
                    *(H+1) = bp_stacktrace_lst;
                    bp_stacktrace_lst = ADDTAG(H, LST);
                    H += 2;
                }
            }
            while (b <= af) {
                RESET_TABLED_SUBGOAL_AR(b);
        ...
        //        write_term(bp_stacktrace_lst); printf("\n");
        CONTCASE;
    }
    ...
    */
{
    ...
    if ((BPLONG)answer_table & 0x1) {  // has one answer 
        ...
        if (opt_arg_index == 0) {
            ...
            int res;
            BPLONG hcode;
            if ((opt_arg_index & 0xffffL) == 0) {  // maximize or minimize
                opt_arg_index >>= 16;
                maximize = 1;
            } else {
                maximize = 0;
            ...
            // come here if res == 0, same objective value
            if (table_card == 1) {
                BACKTRACK;
            } 
    ...
    // answer table has been allocated, may only contain one answer
    answer_table = (BPLONG_PTR)GT_ANSWER_TABLE(subgoal_entry);
    if (opt_arg_index == 0) {
        if (addTableAnswer(stack_arg_ptr, arity, subgoal_entry) == BP_ERROR)
            goto table_error;
        BACKTRACK;
    } else {  // come here only when the opt mode is mmin or mmax
        int res;
        BPLONG hcode;
